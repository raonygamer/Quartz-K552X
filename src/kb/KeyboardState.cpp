...
namespace quartz::kb
{
    ...
    void KeyboardState::debounce(const utils::BitSet<MatrixDefinitions::Size>& rawStates) noexcept
    {
        ...
        const auto* h4 = History[4].data.data();

        for (std::size_t i = 0; i < MatrixBytes; ++i)
        {
            const std::uint8_t pressed = h0[i] & h1[i] & h2[i] & h3[i] & h4[i];
            const std::uint8_t released = static_cast<std::uint8_t>(~(h0[i] | h1[i] | h2[i] | h3[i] | h4[i]));
